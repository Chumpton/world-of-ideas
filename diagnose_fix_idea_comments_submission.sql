-- Diagnose + permanently fix Idea Discussion comments (save + display)
...
-- A) Ensure table shape is exactly what app expects.
...
-- B) RLS: remove policy drift and set one coherent policy set.
...
-- C) Keep ideas.comment_count accurate automatically.
...
-- D) Stable insert RPC that app can call safely.
...
BEGIN
  ...
  SELECT COALESCE(i.comment_count, 0) INTO v_count
  FROM public.ideas i
  WHERE i.id = p_idea_id;

  RETURN QUERY
  SELECT v_row.id, v_row.idea_id, v_row.user_id, v_row.text, v_row.author, v_row.author_avatar, v_row.parent_id, v_row.votes, v_row.created_at, v_count;
END;
$$;
...
CREATE INDEX IF NOT EXISTS idx_idea_comments_idea_created_at ON public.idea_comments (idea_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_idea_comments_parent_id ON public.idea_comments (parent_id);
CREATE INDEX IF NOT EXISTS idx_idea_comments_user_id ON public.idea_comments (user_id);

COMMIT;

-- Quick diagnostics (run after COMMIT):
-- SELECT relrowsecurity FROM pg_class WHERE oid = 'public.idea_comments'::regclass;
-- SELECT policyname, cmd, qual, with_check FROM pg_policies WHERE schemaname='public' AND tablename='idea_comments' ORDER BY policyname;
-- SELECT routine_name, routine_type FROM information_schema.routines WHERE routine_schema='public' AND routine_name='add_idea_comment';
